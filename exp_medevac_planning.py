...
def main(n_casualties, n_assets, n_cfs):
    ...
    algo_mission_assets = FactoryAlgos.create_algo(mode= AlgoName.MEDEVAC_PLAN)
    output_plans = algo_mission_assets.return_medevac_plan(missions_options=all_mission_options, assets=all_assets, care_facilities=all_care_facilities,
                                            settings = settings_test_case)

    assignment_urgency = output_plans.urgency_plan
    assignment_reverse = output_plans.reverse_plan

    if not pd.isna(assignment_urgency):
        print('Urgency/Triage score based Assignment')
        for cas, (asset, cf, ts) in assignment_urgency.items():
            print(f'{cas}: ({asset}, {cf})')
    if not pd.isna(assignment_reverse):
        print('Reverse/ Return time to duty based Assignment')
