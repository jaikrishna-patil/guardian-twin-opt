...
class OptimizationMultipleObjMultipleConstraints(OptimizationMethods):

    def __init__(self):
        self.interpretation = None
        self.next_time = 0

    def use_milp_primary_triage(self, persons, assets, hospitals, required_timesteps, person_score, rtd_timesteps,
                                score_threshold, rtd_threshold):
        print('Printing values:::')
        print(person_score, rtd_timesteps, score_threshold, rtd_threshold)
        # Create a new model
        m = Model()
        ...
        m.objective = maximize(xsum(
            Y[p] + Y[p] * person_score[p] for p in persons) + xsum(
        ...
        for p in persons:
            if person_score[p] >= score_threshold:
                m += xsum(Z[r, p, h] for r in assets for h in hospitals if (p, r, h) in required_timesteps) >= 1
            if rtd_timesteps[p] <= rtd_threshold:
                m += xsum(Z[r, p, h] for r in assets for h in hospitals if (p, r, h) in required_timesteps) >= 1
        ...
        if status == OptimizationStatus.OPTIMAL or status == OptimizationStatus.FEASIBLE:
            ...
            for p in persons:
                ...
                for r in assets:
                    for h in hospitals:
                        if (p, r, h) in required_timesteps and Z[r, p, h].x > 0.99:
    ...
    def use_milp_reverse_triage(self, persons, assets, hospitals, required_timesteps, person_score, rtd_timesteps,
                                score_threshold, rtd_threshold):
        # Create a new model
        m = Model()
        ...
        m.objective = maximize(xsum(
            Y[p] + Y[p] * (1 / (1 + rtd_timesteps[p])) for p in persons) + xsum(
            (1 / (1 + required_timesteps[(p, r, h)])) * Z[r, p, h] for p in persons for r in assets
            for h in hospitals if (p, r, h) in required_timesteps))
        ...
        # New constraint: Ensure every person with person_score >= 0.5 gets an asset assignment
        for p in persons:
            if person_score[p] >= score_threshold:
                m += xsum(Z[r, p, h] for r in assets for h in hospitals if (p, r, h) in required_timesteps) >= 1
            if rtd_timesteps[p] <= rtd_threshold:
    ...
    def use_milp_situational_triage(self, persons, assets, hospitals, required_timesteps,
                                    person_equipments_needed, person_score, rtd_timesteps,
                                score_threshold, rtd_threshold):
        # Create a new model
        m = Model()
    ...
    def get_pyreason_bool(self, python_bool: bool) -> str:
        if python_bool:
            ...
            return '0,0'
    def create_pyreason_graph(self, missions_options: list[MissionOptionsAssets], assets: list[Asset], care_facilities: list[CareFacility], objectives: list, constraints: dict):
        ...
        if 'constraint_triage' in constraints:
            g.add_node('constraint_triage', type_constr_vtol = '1,1')
            for i in range(1, 101):
                g.add_edge('constraint_triage', i, value='0,1')

        if 'constraint_rtd' in constraints:
            g.add_node('constraint_rtd', type_constr_triage_score = '1,1')
            for i in range(1, 101):
                g.add_edge('constraint_rtd', i, value='0,1')
    ...
    def run_opt_problems(self, persons:list, all_assets:list, all_cfs, required_timesteps:dict, person_equipments_needed:dict, run_primary: bool, run_reverse: bool, run_situational: bool, person_score, rtd_timesteps, score_threshold, rtd_threshold):

        primary_feasible, assignment_primary, primary_obj_val, Z_primary, Y_primary = False, None, None, None, None
        reverse_feasible, assignment_reverse, reverse_obj_val, Z_reverse, Y_reverse = False, None, None, None, None
        situational_feasible, assignment_situational, situational_obj_val, Z_situational, Y_situational = False, None, None, None, None

        if run_primary:
            primary_feasible, assignment_primary, primary_obj_val, Z_primary, Y_primary = self.use_milp_primary_triage(persons= persons, assets= all_assets, hospitals= all_cfs, required_timesteps= required_timesteps, person_score= person_score, rtd_timesteps=rtd_timesteps, score_threshold=score_threshold, rtd_threshold=rtd_threshold)
        if run_reverse:
    ...
    def initialize_pyreason(self, missions_options, assets, care_facilities, objectives, constraints):
        pr.reset()
        pr.reset_rules()
        ...
        graph = self.create_pyreason_graph(missions_options=missions_options, assets=assets,
                                           care_facilities=care_facilities, objectives=objectives, constraints= constraints)
    ...
    def frame_opt_problems(self, missions_options, assets, care_facilities, objectives, constraints):
        ...
        all_assets, assets_time_ranges, curr_asset_location, assets_speeds, all_lats_longs, assets_crew_duty_hrs = self.get_assets_dicts(
            assets=assets, assets_time_ranges=assets_time_ranges, curr_asset_location=curr_asset_location, assets_speeds=assets_speeds, all_lats_longs=all_lats_longs, assets_crew_duty_hrs=assets_crew_duty_hrs)
        ...
        for constraint_key, constraint_val in constraints.items():
            ...
            # fact = pr.fact_edge.Fact(f'f_constr',
            #                          (f'constraint_{constraint_key}', str(constraint_val)),
            #                          pr.label.Label('value'),
            #                          pr.interval.closed(1, 1),
            #                          self.next_time,
            #                          self.next_time)
            # edge_facts.append(fact)
            dict_optimization_instances[f'threshold_{constraint_key}'] = constraints[constraint_key]
    ...
    def run_optimization_instances(self, dict_optimization_instances):
        ...
        threshold_score = dict_optimization_instances['threshold_score']
        threshold_rtd = dict_optimization_instances['threshold_rtd']
        ...
        Y_situational) = self.run_opt_problems(persons=persons, all_assets=all_assets, all_cfs=all_cfs,
                                               ...
                                               run_reverse=run_reverse_triage, run_situational=run_situational_triage, rtd_timesteps=person_rtd_ts, score_threshold=threshold_score, rtd_threshold=threshold_rtd
                                               )
    ...
    def run_final_lp(self, dict_optimization_results, constraints, num_iterations):
        ...
        # limit_reached = False
        # for constraint_key, constraint_val in constraints.items():
        #     if constraint_key == 'score':
        #         new_constraint_val = constraints[constraint_key] - 0.1
        #         if new_constraint_val < 0.0:
        #             limit_reached = True
        #             break
        #     elif constraint_key == 'rtd':
        #         new_constraint_val = constraints[constraint_key] + 10
        #         if new_constraint_val >100:
        #             limit_reached = True
        #             break
        ...
        if accepted == 'None':
            # print('Solution found biut not satisfiing min requirement and limit not rached')
            for constraint_key, constraint_val in constraints.items():

                if constraint_key == 'score':
                    new_constraint_val = round(constraints[constraint_key] + 0.1, 10)
                    if new_constraint_val > 1.0:
                        return True, constraints, accepted
                    constraints[constraint_key] = round(constraint_val + 0.1,10)
                elif constraint_key == 'rtd':
                    new_constraint_val = constraints[constraint_key] - 10
                    if new_constraint_val <0:
                        return True, constraints, accepted
                    constraints[constraint_key] = constraint_val-10


            return False, constraints, accepted
        else:
            return True, constraints, accepted
    ...
    def return_final_assignments_multiple_obj_multiple_constraints(self, missions_options: list[MissionOptionsAssets], assets: list[Asset], care_facilities : list[CareFacility], objectives: list, constraints: dict):
        ...
        ((primary_sol_primary_obj, reverse_sol_primary_obj, situational_sol_primary_obj),
         ...
         (primary_sol_situational_obj, reverse_sol_situational_obj, situational_sol_situational_obj)) = (
            (None, None, None), (None, None, None), (None, None, None))
        initial_threshold_score = constraints['score']
        initial_threshold_rtd = constraints['rtd']
        '''
        ...
        self.initialize_pyreason(missions_options=missions_options, assets=assets, care_facilities= care_facilities, objectives= objectives, constraints= constraints)
        num_iterations = 0
        while not terminate:
            dict_optimization_instances = self.frame_opt_problems(missions_options=missions_options, assets=assets, care_facilities= care_facilities, objectives= objectives, constraints= constraints)
            dict_optimization_results = self.run_optimization_instances(dict_optimization_instances = dict_optimization_instances)
            terminate, constraints, accepted = self.run_final_lp(dict_optimization_results=dict_optimization_results, constraints=constraints, num_iterations=num_iterations)
            num_iterations += 1
            print(constraints)
            print(num_iterations)


        threshold_score = constraints['score']
        threshold_rtd = constraints['rtd']
        assignment_primary = dict_optimization_results['assignment_primary']
        assignment_reverse = dict_optimization_results['assignment_reverse']
        assignment_situational = dict_optimization_results['assignment_situational']
        ...
        assignment_final ={}
        if accepted == 'PRIMARY':
            assignment_final = assignment_primary
            ...
            assignment_final = assignment_reverse
            ...
            assignment_final = assignment_situational
        ...
        return (mission_final_assets, num_iterations, accepted, initial_threshold_score, initial_threshold_rtd, threshold_score, threshold_rtd, assignment_final, (assignment_primary, assignment_reverse, assignment_situational),
                (primary_sol_primary_obj, reverse_sol_primary_obj, situational_sol_primary_obj),
                (primary_sol_reverse_obj, reverse_sol_reverse_obj, situational_sol_reverse_obj),
                (primary_sol_situational_obj, reverse_sol_situational_obj, situational_sol_situational_obj))
