...
class OptimizationMultipleObj(OptimizationMethods):
    ...
    def use_milp_situational_triage(self, persons, assets, hospitals, required_timesteps, required_timesteps_lsi_only,
                                    ...
                                    constraint_lsi=True, constraint_air_time=True):
        # Create a new model
        m = Model()
    ...
    def run_primary_objective(self, Z, Y, persons, assets, hospitals, person_score, required_timesteps):
        ...
        # First part: Sum over Y[p] + 10 * Y[p] * person_score[p] for all persons
        for p in persons:
            obj_val += Y[p].x + 10 * Y[p].x * person_score[p]

        # Second part: Subtract 0.1 * Sum of required_timesteps[(p, r, h)] * Z[r, p, h] for valid (p, r, h)
        penalty = 0
        for p in persons:
            for r in assets:
                for h in hospitals:
                    if (p, r, h) in required_timesteps:
                        penalty += required_timesteps[(p, r, h)] * Z[r, p, h].x
    ...
    def run_reverse_objective(self, Z, Y, persons, assets, hospitals, rtd_timesteps, required_timesteps):
        ...
        obj_val = 0
        for p in persons:
            obj_val += 1*Y[p] + 100*Y[p].x * (1 / (1 + rtd_timesteps[p]))
        # Second part: Subtract 0.1 * Sum of required_timesteps[(p, r, h)] * Z[r, p, h] for valid (p, r, h)
        penalty = 0
        for p in persons:
            for r in assets:
    ...
    def run_situational_objective(self, Z, Y, persons, assets, hospitals, person_score, required_timesteps):
        obj_val = 0
    ...
    def create_pyreason_graph(self, missions_options: list[MissionOptionsAssets], assets: list[Asset], care_facilities: list[CareFacility], objectives: list):
        ...
        # if 'vtol' in constraints:
        #     g.add_node('constraint_vtol', type_constr_vtol = '1,1')
        #     for i in range(1, 101):
        #         g.add_edge('constraint_vtol', i, value='0,1')
        #
        # if 'triage_score' in constraints:
        #     g.add_node('constraint_triage_score', type_constr_triage_score = '1,1')
        #     for i in range(1, 101):
        #         g.add_edge('constraint_triage_score', i, value='0,1')
    ...
    def run_opt_problems(self, persons: list, all_assets: list, all_cfs, person_score: dict, person_lsi_ts: dict,
        ...
        assignment_primary, primary_obj_val, Z_primary, Y_primary = None, None, None, None
        assignment_reverse, reverse_obj_val, Z_reverse, Y_reverse = None, None, None, None
        assignment_situational, situational_obj_val, Z_situational, Y_situational = None, None, None, None

        if run_primary:
            assignment_primary, primary_obj_val, Z_primary, Y_primary = self.use_milp_primary_triage(
        ...
        if run_reverse:
            assignment_reverse, reverse_obj_val, Z_reverse, Y_reverse = self.use_milp_reverse_triage(
                persons=persons, assets=all_assets, hospitals=all_cfs, rtd_timesteps=person_rtd_ts,
                required_timesteps=required_timesteps, required_timesteps_lsi_only=required_timesteps_lsi_only,
                person_lsi_ts=person_lsi_ts)
        if run_situational:
            assignment_situational, situational_obj_val, Z_situational, Y_situational = self.use_milp_situational_triage(
                persons=persons, assets=all_assets, hospitals=all_cfs,
                person_score=person_score, required_timesteps=required_timesteps,
                required_timesteps_lsi_only=required_timesteps_lsi_only, person_lsi_ts=person_lsi_ts,
                required_ts_asset_isop=required_ts_asset_isop, required_ts_isop_cf=required_ts_isop_cf)
        return (assignment_primary, primary_obj_val, Z_primary, Y_primary), (assignment_reverse, reverse_obj_val, Z_reverse, Y_reverse), (assignment_situational, situational_obj_val, Z_situational, Y_situational)
    ...
    def run_optimization_instances(self, dict_optimization_instances):
        ...
        ((assignment_primary, primary_obj_val, Z_primary, Y_primary), (assignment_reverse, reverse_obj_val, Z_reverse, Y_reverse),
         (assignment_situational, situational_obj_val, Z_situational, Y_situational)) = self.run_opt_problems(persons=persons, all_assets=all_assets, all_cfs=all_cfs,
                                           person_score=person_score, required_timesteps=required_timesteps,
        ...
        if not assignment_primary or not assignment_reverse or not assignment_situational:
    ...
    def run_final_lp(self, dict_optimization_results):
        # primary_feasible = dict_optimization_results['assignment_primary']
        # reverse_feasible = dict_optimization_results['assignment_primary']
        # situational_feasible = dict_optimization_results['assignment_primary']
        accepted_list_1 = []
        accepted_list_2 = []
        accepted_list_3 = []
        accepted = 'None'
        ...
        if accepted == 'None':
            opt_solution_found = False
        else:
            opt_solution_found = True

        return opt_solution_found, accepted
    ...
    def return_final_assignments_multiple_obj(self, missions_options: list[MissionOptionsAssets], assets: list[Asset], care_facilities : list[CareFacility], objectives: list):
        ...
        dict_optimization_instances = self.frame_opt_problems(missions_options=missions_options, assets=assets, care_facilities= care_facilities)
        ...
        # if solution_found:
        #     opt_solution_found, accepted = self.run_final_lp(dict_optimization_results=dict_optimization_results)
        # else:
        #     opt_solution_found = False
        #     accepted = 'None'
        ...
        assignment_final = {}
        # if solution_found and opt_solution_found:
        #     if accepted == 'PRIMARY':
        ...
        return (mission_final_assets, accepted, assignment_final, (assignment_primary, assignment_reverse, assignment_situational),
