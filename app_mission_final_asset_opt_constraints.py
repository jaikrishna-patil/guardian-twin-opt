...
if __name__ == "__main__":
    ...
    mission1 = MissionOptionsAssets(patient_name='Ben Stokes', location= [48.8584, 2.2945], care_facilities_possible=['Battlefield Medical Center', 'Noble Medical Center'], assets_possible=['Black hawk HH60M', 'Chinook CH47', 'Chinook CH99'], triage_score = 20, available_ts = 4.5, need_equipments=0)
    mission2 = MissionOptionsAssets(patient_name='Steve Smith', location = [47.9931, 8.2454], care_facilities_possible=['Battlefield Medical Center', 'Noble Medical Center'], assets_possible=['Black hawk HH60M', 'Chinook CH47', 'Chinook CH99', ], triage_score = 10, available_ts = 4, need_equipments=1)
    mission3 = MissionOptionsAssets(patient_name='Mike Hussey', location = [46.4868, 2.4128], care_facilities_possible=['Battlefield Medical Center', 'Noble Medical Center'], assets_possible=['Black hawk HH60M', 'Chinook CH47', 'Chinook CH99'], triage_score = 40, available_ts = 8.5, need_equipments =1)
    mission4 = MissionOptionsAssets(patient_name='Viv Richards', location = [49.8675, 0.6713], care_facilities_possible=['Battlefield Medical Center', 'Noble Medical Center'], assets_possible=['Chinook CH99','Black hawk HH60M'], triage_score = 90, available_ts = 0.5, need_equipments =0)
    mission5 = MissionOptionsAssets(patient_name='Sharona Fleming', location = [50.0087, 5.4571], care_facilities_possible=['Battlefield Medical Center', 'Noble Medical Center'], assets_possible=['Chinook CH47'], triage_score = 5, available_ts = 0.5, need_equipments =0)
    ...
    print("")
    print("ALGO :: Mission options assets -> Mission final asset")
    algo_mission_assets = MissionoptionsAssetsToMissionfinalAssetsFactory.create_missionoptionsAssets_to_missionfinalAssets_algo(mode=MissionoptionsAssetsToMissionfinalAssetsAlgoName.OPT_CONSTRAINTS)
    assets_all_missions = algo_mission_assets.return_mission_final_asset_OptConstraints(missions_options=all_mission_options, assets=all_assets, care_facilities=all_care_facilities, weight_available_ts = 5, weight_need_equipment=1)
    # for mission_asset in assets_all_missions:
    #     print('\n')
    #     print(f"Patient {mission_asset.patient_name} is finally assigned following schedule: {mission_asset}")
